...
const addUser = async (req, res) => {
    const key = HASH; 
    const { cedula, nombre, apellido, username, password, correo, rol, genero } = req.body;
    const file = req.file;

    try {
        if (!cedula || !nombre || !apellido || !username || !password || !correo || !rol || !genero) {
            if (file) {
                await unlink(path.join(__dirname, "../static/images/users", file.filename));
            }
            return res.status(400).json({ message: "Bad Request: Por favor, complete todos los campos." });
        }
        ...
        const values = [cedula, nombre, apellido, username, hashPassword, correo, rol, genero];
